...
```{r}
# Recoding hatlev1d as 3 indicator variables
...
rifr_med <- dineq::rifr(hwusual ~ age + (sex - 1) + low_educ + high_educ, eulfs_small, weights = NULL, method = "quantile", quantile = 0.5, kernel = "gaussian")
```
...
table_rifr(rifr_med, "RIF Regression using the median: usual working hours regressed on age, sex and education", c("Age", "Female", "Low education", "High education"))
```
...
\begin{table}[!htbp] \centering
 ...
 \hline \\[-1.8ex]
  Age  &  0.5050 $^{ *** }$ \\ 
    ...
    & ( 0.0419 ) \\ 
    & \\
      Low education  &  2.5036 $^{ *** }$ \\ 
    & ( 0.0672 ) \\ 
    & \\
      High education  &  2.2237 $^{ *** }$ \\ 
    & ( 0.0654 ) \\ 
    & \\
    \hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{1}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
...
All coefficients are still very significant.
The coefficient associated with age is almost unchanged, the one associated with being a woman is higher, and the coefficient associated with low education is now higher than the one associated with high education (the reverse of the previous result).

```{r}
moy_hw <- mean(eulfs_small$hwusual, na.rm =TRUE)
med_hw <- quantile(eulfs_small$hwusual, probs =0.5, na.rm =TRUE)
cat("The mean,", format(round(moy_hw, 4), nsmall = 4), ", is actually lower than the median,", format(round(med_hw, 4), nsmall = 4), "; which means that there are extreme cases, in particular with low values.")

```

```{r}
# Eliminating people who usually do not work (hwusual = 0 is treated as NA)
eulfs_no0 <- eulfs_small
eulfs_no0$hwusual <- na_if(eulfs_small$hwusual, 0)
moy_hw <- mean(eulfs_no0$hwusual, na.rm =TRUE)
med_hw <- quantile(eulfs_no0$hwusual, probs =0.5, na.rm =TRUE)
cat("Even after eliminating people with 0 working hours, the mean,", format(round(moy_hw, 4), nsmall = 4), ", is lower than the median,", format(round(med_hw, 4), nsmall = 4), "; which means that there are extreme cases, in particular with low values.")
```

The question is now: how to correctly interpret the difference between those two regressions?
I would say that extreme values (very low and very high number of working hours) have a non-negligible effect on the mean, and therefore on the linear regression.
When we reduce the weight of people working only very few hours, we get slightly different results. For instance, the proportion of female workers among those who work a few hours is high. Thus, the coefficient associated with "Female" is higher when we take the median as a reference like in the RIF regression.
...
rifr_1998 <- dineq::rifr(hwusual ~ age + (sex - 1) + low_educ + high_educ, eulfs1998, weights = NULL, method = "quantile", quantile = 0.25, kernel = "gaussian")
rifr_2013 <- dineq::rifr(hwusual ~ age + (sex - 1) + low_educ + high_educ, eulfs2013, weights = NULL, method = "quantile", quantile = 0.25, kernel = "gaussian")
```
